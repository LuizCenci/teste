...
CREATE OR REPLACE FUNCTION sel() RETURNS FLOAT
AS $$
DECLARE 
	tempo FLOAT;
	total TEXT;
BEGIN
		DISCARD PLANS;
    	EXPLAIN (ANALYZE, TIMING, FORMAT JSON) into total SELECT Nome FROM Cliente WHERE CliId = '1532';
		tempo := ((total::jsonb)-> 0 -> 'Execution Time')::FLOAT;
    	INSERT INTO tempos VALUES (tempo);
		RETURN tempo;
END;
$$  LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION print() RETURNS TABLE (printar float) AS $$
DECLARE 
    i INTEGER;
BEGIN
    FOR i IN 1..100 LOOP
